...
# w = WorkspaceClient()
# spark.sql('GRANT USAGE ON CATALOG `leon_eller` TO `68df7d8e-8bf7-43d3-b55d-534855fb1c42`');
# spark.sql('GRANT USAGE ON DATABASE `leon_eller`.`nuix_chatbot` TO `68df7d8e-8bf7-43d3-b55d-534855fb1c42`');
# w.grants.update(c.SecurableType.TABLE, "leon_eller.nuix_chatbot.documentation_vs_index",
# changes=[c.PermissionsChange(add=[c.Privilege["SELECT"]], principal="68df7d8e-8bf7-43d3-b55d-534855fb1c42")])
# w.grants.update(c.SecurableType.SCHEMA, "leon_eller.nuix_chatbot", changes=[c.PermissionsChange(add=[c.Privilege["EXECUTE"]], principal="68df7d8e-8bf7-43d3-b55d-534855fb1c42")])
...
TEMPLATE = """You are an assistant for SafetyCulture customers. You are answering all questions related to SafetyCulture and SafetyCulture Application and Inspection Reports. If the question is not related to one of these topics, kindly decline to answer. If you don't know the answer, just say that you don't know, don't try to make up an answer. Keep the answer as detailed as possible.
...
question = {"query": "What does SafetyCulture do?"}
...
model_name = f"{catalog}.{db}.safety-culture-chatbot-model"
...
question = "How can I avoid trips?"
...
# demo = gr.Interface(fn=greet, inputs=[gr.Textbox(label="Question", lines=2)], outputs=[gr.Textbox(label="Answer", lines=50, min_width=400)])
...
# cleanup_demo(catalog, db, serving_endpoint_name, f"{catalog}.{db}.documentation_vs_index")
