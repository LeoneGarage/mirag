...
sitemap_urls = [u.strip() for u in dbutils.widgets.get("sitemap_urls").split(",") if u.strip()]

# COMMAND ----------

if len(sitemap_urls)> 0 and (not table_exists("raw_documentation") or spark.table("raw_documentation").isEmpty()):
...
# MAGIC
# MAGIC The last step is to apply our UDF all our documentation text and save them to our `documentation` table
# MAGIC
...
# MAGIC CREATE TABLE IF NOT EXISTS web_documentation (
...
if len(sitemap_urls) > 0 and (not table_exists("web_documentation") or spark.table("web_documentation").isEmpty()):
    (
        ...
        .saveAsTable("web_documentation")
    )

display(spark.table("web_documentation"))
