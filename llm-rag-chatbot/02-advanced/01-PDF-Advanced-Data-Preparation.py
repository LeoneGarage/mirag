...
# MAGIC %pip install transformers==4.30.2 "unstructured[pdf,docx]==0.10.30" langchain llama-index databricks-vectorsearch pydantic==1.10.9 mlflow
...
# MAGIC %sql
# MAGIC
# MAGIC CREATE TABLE IF NOT EXISTS pdf_raw (
# MAGIC   path STRING,
...
df = (spark.readStream
        ...
        # .option("cloudFiles.allowOverwrites", "true") # Uncomment this line if you also want autoloader to detect changed files with the same name see https://learn.microsoft.com/en-us/azure/databricks/ingestion/cloud-object-storage/auto-loader/faq#process-file-behavior for caveats
        .load('dbfs:'+volume_folder+"/pdfs"))
...
# CREATE TABLE IF NOT EXISTS pdf_documentation (
...
def upsert_chunks(batch_df, batch_id):
    deltaTable = DeltaTable.forName(spark, "pdf_documentation")
    target_df = deltaTable.toDF().select("url", "chunk_index", "content")
    window = Window.partitionBy("url", "chunk_index").orderBy(col("_commit_version").desc())
    ...
    batch_df = batch_df.select(col("*"), row_number().over(window).alias("rn"), max(col("chunk_index")).over(window_max).alias("max_chunk_index")).where(col("rn") == 1).drop("rn").drop("_commit_version")
    batch_df = batch_df.unionByName(
        ...
        target_df.join(batch_df, on=(batch_df.url == target_df.url) & (batch_df.max_chunk_index < target_df.chunk_index), how="left_semi").withColumn("op", lit("D")).withColumn("max_chunk_index", lit(None))
    ).drop("max_chunk_index")
    # now perform the merge
    (
        deltaTable.alias('target')
            .merge(
                batch_df.alias('source'),
                    'target.url = source.url AND target.chunk_index = source.chunk_index')
            .whenMatchedDelete(condition = "source.op = 'D'")
            .whenMatchedUpdate(set = {"url": "source.url", "chunk_index": "source.chunk_index", "content":"source.content"})
            .whenNotMatchedInsert(condition = "source.op != 'D'", values = {"url": "source.url", "chunk_index": "source.chunk_index", "content":"source.content"})
            .execute()
    )

# COMMAND ----------

(spark.readStream.option("readChangeFeed", "true").table('pdf_raw')
      .where("_change_type != 'update_preimage'")
      .select(
        col("path").alias("url"),
        posexplode(read_as_chunk("content")).alias("chunk_index", "content"),
...
# %sql
# SELECT * FROM documentation WHERE url like '%.pdf' limit 10
...
# source_table_fullname = f"{catalog}.{db}.pdf_documentation"
